...
public class DNS {
	...
	public static final List<Pair<String, Integer>> getMXRecords(String domainname) {
		...
		for (indexOfAnswer += 1; indexOfAnswer < lines.length; indexOfAnswer++) {
			...
			// winxp:
			// "kataf.nl        MX preference = 10, mail exchanger = mail.kataf.nl"

			int priority;

			if (line.contains("Non-authoritative answer:")) // for linux, for winxp
																			// this is in stderr
				continue;
	...
	private static void randomizeRecordsKeepPriority(List<Pair<String, Integer>> domainPriorities) {
		Collections.shuffle(domainPriorities); // to randomize domains with same
															// priority later on
	...
	private static final String[] query(String domainname, String queryType) {
		try {
			String exec = "nslookup";
			if (!System.getProperty("os.name").toLowerCase().contains("win"))
				exec = "/usr/bin/" + exec;
			
			Duo<byte[]> std = HighLevel.executeProcess(exec, "-q=" + queryType, domainname);
	...
	public static final String queryHostname() {
		try {
			String exec = "hostname";
			if (!System.getProperty("os.name").toLowerCase().contains("win"))
				exec = "/bin/" + exec;
			
			Duo<byte[]> std = HighLevel.executeProcess(exec);
			byte[] stdout = std.first();
			// byte[] stderr = std.second();
			// if (stderr.length != 0)
			// System.err.println(Text.ascii(stderr));
